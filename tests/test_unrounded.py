...
def encode_categorical_features_modified(df: pd.DataFrame) -> pd.DataFrame:
    ...
    dummies = pd.get_dummies(df[["Produk", "Kategori"]], drop_first=False).astype(int)
...
for i in range(10):
    ...
    curr_encoded = encode_categorical_features_modified(curr_features)
    ...
    for p_idx, p_name in enumerate(prod_names):
        pred_val = preds[p_idx]  # UNROUNDED
        mask = (current_df['Tanggal'] == next_date) & (current_df['Produk'] == p_name)
        current_df.loc[mask, 'Qty_Terjual'] = round(pred_val)
        
        future_rows.append({
            'Tanggal': next_date,
            'Produk': p_name,
            'Unrounded_Pred': pred_val
        })

res_df = pd.DataFrame(future_rows)
print("Teh Celup Unrounded:")
print(res_df[res_df['Produk'] == 'Teh Celup']['Unrounded_Pred'].tolist())

print("Kopi Botol Unrounded:")
print(res_df[res_df['Produk'] == 'Kopi Botol']['Unrounded_Pred'].tolist())
